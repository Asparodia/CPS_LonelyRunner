...
public class EditableScreenContract extends ScreenContract implements EditableScreenService {
	...
	public void setNature(int x, int y, Cell c) {
		if (!(x >= 0 && x < getWidth())) {
			throw new PreconditionError("setNature(" + x + ", " + y + " c )",
		...
		if (!(getDelegate().getCellNature(x, y) == c)) {
			throw new PostconditionError("setNature(" + x + ", " + y + " c )",
					" cellNature of " + x + " " + y + " didn't change ");
		}
		for (int a = 0; a < getWidth(); a++) {
			for (int b = 0; b < getHeight(); b++) {
				if (a != x || b != y) {
					...
					if (nc != null) {
						if (!(getCellNature(a, b) == nc)) {
							throw new PostconditionError("setNature(" + x + ", " + y + " c )",
									"cellNature of " + x + " " + y + " changed, wasn't suppose to change ");
						}
					}
				}
			}
